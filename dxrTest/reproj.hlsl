...
void main(uint3 DTid : SV_DispatchThreadID) {
  ...
  // то есть у него должна отличатьс€ глубина? или что?
  ...
  // считать веса 
  // веса это 4 точки вокруг текущего значени€ projectedCoords
  ...
  // мы берем несколько точек, вычисл€ем веса (дл€ исторического значени€), складываем их
  ...
  // у нас есть texelSize (размер пиксел€), по идее мне нужно разделить одно на другое
  // пиксель индекс ???
  ...
  for (uint k = 0; k < 4; ++k) {
    ...
    // коэффициент должен быть св€зан с texelSize
  ...
  //  // а теперь что? вз€ть средний цвет? (эт вообще жизнеспособна€ иде€, но она мне чет не нравитс€)
...
//
//  // првильно ли € скопировал? где то должна быть и матрица
